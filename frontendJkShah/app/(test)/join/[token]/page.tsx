...
const SCRUBBED_JOIN_PATH = "/join/joined";
...
export default function JoinTestPage() {
  ...
  // Resolved once, then scrubbed out of the URL so it never lingers in
  // browser history, Referer headers or logs (HIGH-03).
  ...
  useEffect(() => {
    if (isFullShareLink) {
      ...
      // Legacy bare-token links (/join/<token>)
      setToken(rawToken);
    }
